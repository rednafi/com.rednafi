	...
	for _, viewport := range []playwright.Size{
		...
		t.Run(fmt.Sprintf("%dx%d", viewport.Width, viewport.Height), func(t *testing.T) {
			...
			problems, err := page.Evaluate(`() => {
				...
				checkType(".article-content > p", mobile ? 16 : 18, mobile ? 24 : 28, "article body");
				...
				// Sole typography exception: fenced code uses the requested larger custom
				// scale. Every other assertion in this test remains Vercel-exact.
				...
				const expectedWidth = Math.min(innerWidth - 48, 720);
				...
				if (mobile) close(style("main").paddingLeft, 24, "mobile rail");
