---
title: Repository pattern & transactions in Go
date: 2026-03-21
slug: repository-pattern-and-transactions
tags:
    - Go
    - Database
    - Design Patterns
...
All code examples use SQLite so you can clone the [companion repo] and run them locally with
...
The `var _ Store = (*memStore)(nil)` line is a compile-time check. If `memStore` ever
...
`database/sql`. After starting the transaction with `BeginTx`, it builds a fresh `Store`
...
This is enough to test service logic: did `RegisterBook` call both `Create` and
`CreateAuditLog`? Did it handle errors correctly?
...
`failingStore` embeds the real SQLite store but overrides `CreateAuditLog` to always return
...
```go
// Don't do this.
func (s *Store) Create(ctx context.Context, b Book) (int64, error) {
    var executor DBTX
...
There's a subtler problem too. Context values are untyped and invisible. If someone forgets
...
transactional store is passed as a function argument, and if you don't use it, the compiler
tells you.
...
Fair question. Go leans toward simplicity. The standard library uses concrete types. A
repository interface, a unit of work, and a `Stores` struct can read like Java patterns
transplanted into Go.

For a small project, it probably is too much. If your service has three endpoints and one
database table, just inject `*sql.DB` and write your queries.

But past a certain codebase size, the simple route turns into a mess. Every half-million-line
codebase I've worked in that skipped these boundaries ended up with SQL scattered across the
service layer, untestable business logic, and transaction management copy-pasted into every
handler. The "just use `*sql.DB` everywhere" approach works until it doesn't, and by that
point the refactor is painful.

The test is practical: does this abstraction make the code easier to change and test? If your
service has multiple stores, needs transactions across them, and has a test suite that should
run without a database, the interface and unit of work pay for themselves. If not, skip them.

One more thing. With LLMs, writing better code from the start has never been easier. Learning
the right patterns and applying them used to take more effort than just hacking something
together. That gap is smaller now. Taking the time to learn the better approach and doing it
properly is more accessible than ever, so we might as well do it.

---

The three patterns build on each other. Start with plain repository interfaces. Add per-store
`Tx` when you need single-store atomicity. Move to a unit of work when you need cross-store
atomicity. Each step is a small refactor from the previous one.

Working examples for the [single-store version] and the [cross-store version] are on GitHub.
...
[companion repo]:
    https://github.com/rednafi/examples
...
[what's a computer?]:
    https://media1.tenor.com/m/NXucPn8GUucAAAAC/computer-whats-a-computer.gif
