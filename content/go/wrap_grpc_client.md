...
The server embeds `UnimplementedKVServer`, which provides no-op defaults for every RPC so
the code compiles even if you haven't implemented all of them yet. The `Get` method checks
the map and returns `codes.NotFound` when the key isn't there. This is the status code the
wrapper will catch and turn into a Go error.

## Using the generated client directly

Without a wrapper, callers use the generated `KVClient` directly to call the server. Pay
attention to the imports:
