...
URLs from the previous domain to the new one and submitting the new [sitemap] to the Search
Console, Google kept indexing the older domain. To make things worse, the search engine
selected the previous domain as canonical, and no amount of manual requests were changing
the status in the last 30 days. Strangely, I didn't encounter this issue with Bing, as it
reindexed the new site within a week after I submitted the sitemap file via their webmaster
panel.
...
This has one big advantage — Google [prioritizes API indexing requests] over sitemap-based
indexing.
...
In my case, this site's [sitemap] lists out all the URLs as follows:
...
[prioritizes API indexing requests]:
