...
hash function is [applied to build the cache key] for all the parameters of the target
function to build the key of the dictionary, and the value is the return value of the
function when those parameters are the inputs. This means, the first argument `self` also
gets included while building the cache key. However, for different instances, this `self`
object is going to be different and that makes the hashed key of the cache different for
every instance even if the other parameters are the same.
...
- [Python LRU cache in a class disregards maxsize limit]

<!-- references -->
<!-- prettier-ignore-start -->

[applied to build the cache key]:
