...
At my workplace, while working on an [AWS Lambda] function, I noticed that my Python logs
weren't appearing on the corresponding [CloudWatch] log dashboard. At first, I thought that
the function wasn't picking up the correct log level from the environment variables. We were
using the [Serverless Framework] and GitLab CI to deploy the function, so my first line of
...
According to the [AWS Lambda function logging docs], to make your logging work in the Lambda
...
[pre-configures a logging handler]:
...
[aws lambda function logging docs]:
